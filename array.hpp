...
public:
    ...
    requires(std::is_assignable_v<value_type&, decltype(a)>)
  {
    ...
    { // [f, i) is moved backwards
      ...
      j.n_ = (std::is_constant_evaluated() ?
        std::move(iterator{this, f}, j, begin()) :
        std::move(E, iterator{this, f}, j, begin())).n_;
    }
    else
  ...
  {
    if (i.n_ <= j.n_)
      nicesort::sort<E>(i.n_, j.n_, cmp);
    else
    {
      nicesort::sort<E>(i.n_, std::addressof(a_[N]), cmp);
      nicesort::sort<E>(a_, j.n_, cmp);
      if ((j.n_ != a_) && cmp(*a_, a_[CAP]))
        std::inplace_merge(E, iterator{this, i.n_}, {this, a_},
          {this, j.n_}, cmp);
    }
